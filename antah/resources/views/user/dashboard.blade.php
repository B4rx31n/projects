...
<div style="max-width: 1200px; margin: 0 auto; padding: 20px;">
    <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 30px; border-radius: 10px; margin-bottom: 30px; color: white;">
        <h1 style="margin: 0 0 10px 0; font-size: 28px;">
            <i class="fas fa-tachometer-alt" style="margin-right: 10px;"></i>User Dashboard
        </h1>
        <p style="margin: 0; opacity: 0.9;">Selamat datang, {{ Auth::user()->name }}! - Dashboard UKK Management</p>
    </div>
    ...
    <div style="text-align: center; padding: 20px;">
        <form method="POST" action="{{ route('user.logout') }}" style="display: inline-block;">
            @csrf
